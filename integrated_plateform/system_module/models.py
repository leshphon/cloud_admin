...
def create_user_operation(request):
    ...
    print("create user _ role id",role)
    user_obj = auth_models.User(name=name, password=make_password(password), email=email,department=department,recent_use_VDC=0)
    user_obj.save()
